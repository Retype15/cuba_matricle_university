...
# add css to streamlit app
    ...
    div.element-container:has(div.float) {
        /* This rule helps hide the original placeholder */
    ...
    div:has( >.element-container div.float) {
        /* This is the main rule that floats the container */
        display: flex;
    ...
    if css is not None:
        ...
        js_ = f'''
            <script>
                ...
                const floatEl = parent.document.querySelector('div.flt-{new_id}');
                if (floatEl) {{
                    // Busca el stVerticalBlock que contiene el elemento. Este es el contenedor real del layout.
                    const blockToFloat = floatEl.closest('[data-testid^="stVerticalBlock"]');
                    if (blockToFloat) {{
                        // Aplica los estilos flotantes y asegura position:fixed.
                        blockToFloat.id = "float-this-component-{new_id}";
                        blockToFloat.style.cssText = `{css} position: fixed; z-index: 99;`;
                    }}
                }}
            </script>
            '''
        st.components.v1.html(js_, height=0)
    ...
    if css is not None:
        ...
        js_ = f'''
            <script>
                ...
                const floatElDelta = parent.document.querySelector('div.flt-{new_id}');
                if (floatElDelta) {{
                    const blockToFloat = floatElDelta.closest('[data-testid^="stVerticalBlock"]');
                    if (blockToFloat) {{
                        blockToFloat.id = "float-this-component-{new_id}";
                        blockToFloat.style.cssText = `display:flex; flex-direction:column; position:fixed; z-index:99; {css}`;
                    }}
                }}
            </script>
            '''
